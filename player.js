...
class SlatePlayer {
  ...
  constructor() {
    ...
    // Opaque code-block runners: id -> iframe (resize bridge is player-lifetime)
    this.codeBlockFrames = new Map()
    this._codeBlockBridgeBound = false
  ...
  applyFontSettings(headingFontId, bodyFontId, headingFontWeight, bodyFontWeight) {
    ...
    // Coerce/sanitize every interpolated field — course JSONB is attacker-controlled
    // for anyone with edit access (or who can poison an import). textContent avoids
    // HTML parse, but allowlisting still blocks CSS breakouts and keeps parity with
    // builder generateFontFaceCss (preview/export <style> string path).
    ...
    if (customFonts && Array.isArray(customFonts)) {
      for (const customFont of customFonts) {
        if (!customFont || typeof customFont !== 'object') continue
        const fontId = typeof customFont.fontId === 'string' ? customFont.fontId : ''
        if (!fontId || !/^custom-[a-z0-9-]+$/i.test(fontId)) continue

        const safeFamilyName = this.sanitizeFontFaceFamilyName(customFont.familyName)
        if (!safeFamilyName) continue

        // Register in fontMap for lookup (family string is CSS, not HTML)
        const familyCss = typeof customFont.family === 'string' && customFont.family
          ? customFont.family
          : `'${safeFamilyName}', sans-serif`
        fontMap[fontId] = familyCss

        // Inject @font-face CSS (only once per font)
        const styleId = `slate-font-${fontId}`
        if (!document.getElementById(styleId)) {
          const style = document.createElement('style')
          style.id = styleId
          let css = ''
          const variants = Array.isArray(customFont.variants) ? customFont.variants : []
          for (const variant of variants) {
            if (!variant || typeof variant !== 'object') continue
            const url = this.sanitizeFontFaceUrl(variant.url)
            if (!url) continue
            const format = this.coerceFontFormat(variant.format)
            const weight = this.coerceFontWeight(variant.weight)
            const fontStyle = this.coerceFontStyle(variant.style)
            css += `@font-face {
  ...
  renderBlock(block, options = {}) {
    ...
    switch (block.type) {
      ...
      case 'code':
        ...
        // Nested Slate blocks stay in the outer player document only when the
        // code block does not use the opaque srcdoc runner (blocks mode, no JS).
        if (block.content.blocks?.length && !this.codeBlockUsesOpaqueFrame(block.content)) {
  ...
  /**
   * html/snippet modes always use an opaque-origin srcdoc iframe.
   * blocks mode uses the iframe only when author JS is present (so nested
   * Slate blocks can keep player init when there is no custom script).
   */
  codeBlockUsesOpaqueFrame(content) {
    if (!content) return false
    if (content.mode === 'html' || content.mode === 'snippet') return true
    return !!(content.js && String(content.js).trim())
  }

  /** Sandbox tokens for the code-block runner — never include allow-same-origin. */
  static get CODE_BLOCK_FRAME_SANDBOX() {
    return 'allow-scripts allow-forms allow-popups allow-popups-to-escape-sandbox allow-downloads'
  }

  /** Max iframe height from child resize messages (px). Caps UI DoS via spoofed postMessage. */
  static get CODE_BLOCK_MAX_FRAME_HEIGHT() {
    return 50000
  }
  ...
  renderCode(content) {
    ...
    if (this.codeBlockUsesOpaqueFrame(content)) {
      // Host + empty iframe; srcdoc is applied in initCode so theme vars and
      // the course <base> href are read from the live player document.
      // Eager load (no loading=lazy): a zero-height iframe may never intersect
      // and would stay blank if lazy-deferred.
      const sandbox = SlatePlayer.CODE_BLOCK_FRAME_SANDBOX
      return (
        `<div class="code-block-frame-host" data-code-block-id="${containerId}">` +
        `<iframe class="code-block-frame" title="Interactive content" ` +
        `sandbox="${sandbox}"></iframe>` +
        `</div>`
      )
    }

    // Blocks mode without JS: keep nested Slate blocks in the player document
    // so accordion/tabs/etc. still receive initNestedBlocks.
  ...
  /** Strip CSS-dangerous characters from a font family name (mirrors builder sanitizeFontName). */
  sanitizeFontFaceFamilyName(name) {
  ...
  /** Allowlist CSS format() tokens used in @font-face (mirrors builder coerceFontFormat). */
  coerceFontFormat(format) {
    const allowed = ['woff2', 'woff', 'truetype', 'opentype']
    return typeof format === 'string' && allowed.includes(format) ? format : 'woff2'
  }

  /** Coerce font-weight to 100–900 step 100 (mirrors builder coerceFontWeight). */
  coerceFontWeight(weight) {
    const n = typeof weight === 'number'
      ? weight
      : typeof weight === 'string' && /^-?\d+$/.test(weight.trim())
        ? Number(weight.trim())
        : NaN
    if (Number.isInteger(n) && n >= 100 && n <= 900 && n % 100 === 0) return n
    return 400
  }

  /** Coerce font-style to normal | italic (mirrors builder coerceFontStyle). */
  coerceFontStyle(style) {
    return style === 'italic' ? 'italic' : 'normal'
  }

  /**
   * Sanitize a font URL for url('...') inside @font-face CSS.
   * Neutralizes quote breakout + CSS-meta patterns (mirrors builder sanitizeFontFaceUrl).
   */
  sanitizeFontFaceUrl(url) {
    if (typeof url !== 'string' || !url) return ''
    const escaped = url.replace(/'/g, '%27').replace(/javascript:/gi, 'blocked:')
    // Mirror builder sanitizeCssMetaValue: CSS-meta filters, control chars, then
    // strip <> so a neutralized </style rewrite cannot leave <script>/<style>.
    return this.sanitizeCss(escaped)
      .replace(/[\u0000-\u001f\u007f]/g, '')
      .replace(/[<>]/g, '')
  }
  ...
  /**
   * Restrict computed theme values before interpolating into srcdoc CSS.
   * Blocks `}</style>` / expression-style breakouts if a variable were ever poisoned.
   */
  sanitizeCodeBlockCssVarValue(value, fallback = '') {
    const raw = (value == null ? '' : String(value)).trim()
    const candidate = raw || fallback
    if (!candidate) return ''
    if (candidate.length > 128) return fallback
    if (/[;{}]|<\/|expression/i.test(candidate)) return fallback
    // Allow typical computed colors/lengths: #hex, rgb()/rgba(), hsl(), numbers, units.
    if (!/^[#a-zA-Z0-9%,.\s()/+-]+$/.test(candidate)) return fallback
    return candidate
  }

  /** Theme CSS custom properties for injection into the opaque code-block document. */
  getCodeBlockThemeCssText() {
    const root = document.documentElement
    const style = getComputedStyle(root)
    const pairs = []
    const set = (name, value, fallback = '') => {
      const v = this.sanitizeCodeBlockCssVarValue(value, fallback)
    ...
    ;[
      '--primary-color', '--accent-color', '--accent-hover',
      '--radius-base', '--outline-color'
    ].forEach(name => set(name, style.getPropertyValue(name)))

    set('--border-radius', style.getPropertyValue('--radius-base'))
    set('--hover-color', style.getPropertyValue('--accent-hover'))
    pairs.push('--success-color:#22c55e', '--error-color:#ef4444')
    set('--text-color', style.getPropertyValue('--slate-800'), '#1e293b')
    set('--text-muted', style.getPropertyValue('--slate-500'), '#64748b')
    set('--bg-light', style.getPropertyValue('--slate-50'), '#f8fafc')
    set('--bg-muted', style.getPropertyValue('--slate-100'), '#f1f5f9')
    set('--border-color', style.getPropertyValue('--slate-200'), '#e2e8f0')
    return pairs.join(';')
  }

  /**
   * Sanitize + assemble the HTML body that goes inside the opaque runner.
   * For blocks+JS, nested blocks are serialized as static HTML (player widget
   * init does not run inside the iframe — author JS owns interactivity).
   */
  getCodeBlockInnerHtml(content) {
    if (content.mode === 'blocks') {
  ...
  /**
   * Build srcdoc for the opaque code-block iframe.
   * Trust boundary: sandbox without allow-same-origin → unique opaque origin,
   * so ownerDocument/defaultView/localStorage cannot reach the builder session
   * even when the outer preview iframe has allow-same-origin.
   */
  buildCodeBlockSrcdoc(content, containerId) {
    const themeCss = this.getCodeBlockThemeCssText()
    const scopedCss = content.css ? this.scopeCss(content.css, `#${containerId}`) : ''
    const innerHtml = this.getCodeBlockInnerHtml(content)
    const userJs = (content.js && String(content.js).trim()) ? String(content.js) : ''
    const useJquery = !!content.useJquery && !!userJs

    // Relative media in SCORM packages must resolve against the course document.
    let baseHref = ''
    try {
      baseHref = new URL('.', document.baseURI || document.URL).href
    } catch {
      baseHref = ''
    }
    ...
    // Keep network/storage surface minimal while allowing images/fonts and the
    // optional jQuery CDN (same dependency the previous player path loaded).
    // connect-src 'none' matches the historical Proxy sandbox (fetch/XHR blocked).
    // No 'unsafe-eval': user JS is injected via <script textContent>, not Function().
    const jquerySrc = useJquery ? ' https://code.jquery.com' : ''
    // 'none' must not be mixed with other base-uri sources (CSP ignores the rest).
    const baseUri = baseHref ? baseHref : "'none'"
    const csp = [
      "default-src 'none'",
      ...
      `script-src 'unsafe-inline'${jquerySrc}`,
      "connect-src 'none'",
      "frame-src 'none'",
      "object-src 'none'",
      `base-uri ${baseUri}`,
    ...
    // JSON.stringify is safe inside a script element once `<` is \u003c-escaped
    // (prevents script-tag breakout from author JS strings).
    // IMPORTANT: never write a literal "</scr" + "ipt>" sequence in this file —
    // builder preview inlines player.js into an HTML <script> block.
    const safeJson = (value) => JSON.stringify(value).replace(/</g, '\\u003c')
    const userJsLiteral = safeJson(userJs)
    const frameIdLiteral = safeJson(containerId)

    const jqueryLoader = useJquery
      ? `
function __slateLoadJquery() {
  return new Promise(function(resolve, reject) {
    ...
    s.onerror = function() { reject(new Error('Failed to load jQuery')); };
    document.head.appendChild(s);
  });
}
`
      : `
function __slateLoadJquery() { return Promise.resolve(); }
`

    // Bootstrap runs in the opaque document. Height posts to parent; user JS
    // receives `container` (API-compatible). parent/top/opener are shadowed
    // via an IIFE parameter list — no Function()/eval (CSP-safe).
    const bootstrap = `
${jqueryLoader}
(function() {
  var frameId = ${frameIdLiteral};
  ...
  function postHeight() {
    try {
      ...
      parent.postMessage({
        source: 'slate-code-block',
        type: 'resize',
        id: frameId,
        height: Math.ceil(h)
      }, '*');
    } catch (e) { /* parent may be inaccessible in exotic embeds */ }
  }
  ...
  function runUserJs() {
    try {
      // Shadow frame-escape and persistence/network globals via IIFE params.
      // Do not use eval as a parameter name (SyntaxError in browsers).
      var paramNames = [
        'container',
        'parent', 'top', 'opener', 'frames',
      ...
      if (window.jQuery) {
        paramNames = ['container', '$', 'jQuery'].concat(paramNames.slice(1));
        ...
        var safeStatic = ['each','map','grep','extend','isArray','isFunction','isPlainObject','trim','type','now','parseJSON','noop','merge','makeArray','inArray','proxy'];
        for (var i = 0; i < safeStatic.length; i++) {
          var key = safeStatic[i];
          if (typeof window.jQuery[key] === 'function') {
            $[key] = window.jQuery[key].bind(window.jQuery);
          }
        }
        paramValues = [container, $, $].concat(paramValues.slice(1));
      }
      // Inject via classic script textContent (no eval / new Function — CSP-safe).
      var s = document.createElement('script');
      s.textContent =
        '(function(' + paramNames.join(',') + '){\\n' +
        userJs +
        '\\n})(' + paramNames.map(function(_, idx) {
          return '__slateArgs[' + idx + ']'
        }).join(',') + ');';
      window.__slateArgs = paramValues;
      document.body.appendChild(s);
      s.remove();
      try { delete window.__slateArgs; } catch (e) { window.__slateArgs = undefined; }
    } catch (err) {
  ...
  __slateLoadJquery().then(runUserJs).catch(function(err) {
    console.error('[Code Block] jQuery failed to load:', err && err.message ? err.message : err);
    runUserJs();
  });
})();
`
    ...
    return (
      ...
      (scopedCss || '') +
      `</style></head><body>` +
      `<div id="${containerId}" class="code-block-container">${innerHtml}</div>` +
  ...
  initCode(wrapper, content) {
    const frame = wrapper.querySelector('iframe.code-block-frame')
    if (frame) {
      this.initCodeOpaqueFrame(frame, content)
      return
    }

    // blocks mode without JS — theme vars only; no script execution
  ...
  /**
   * Single player-lifetime resize bridge. Avoids per-iframe window listeners
   * that leak when lesson re-renders replace the DOM.
   */
  ensureCodeBlockMessageBridge() {
    if (this._codeBlockBridgeBound) return
    this._codeBlockBridgeBound = true
    if (!this.codeBlockFrames) this.codeBlockFrames = new Map()

    const maxHeight = SlatePlayer.CODE_BLOCK_MAX_FRAME_HEIGHT
    const handler = (event) => {
      const data = event.data
      if (!data || data.source !== 'slate-code-block' || data.type !== 'resize') return
      const id = data.id
      if (typeof id !== 'string' || !id) return
      const frame = this.codeBlockFrames.get(id)
      if (!frame) return
      if (!frame.isConnected) {
        this.codeBlockFrames.delete(id)
        return
      }
      if (event.source !== frame.contentWindow) return
      const height = Number(data.height)
      if (!Number.isFinite(height) || height < 0) return
      const capped = Math.min(Math.ceil(height), maxHeight)
      frame.style.height = `${capped}px`
    }
    window.addEventListener('message', handler)
  }

  initCodeOpaqueFrame(frame, content) {
    const host = frame.closest('.code-block-frame-host')
    const containerId = host?.dataset?.codeBlockId || `code-block-${Math.random().toString(36).substr(2, 9)}`

    this.ensureCodeBlockMessageBridge()
    if (!this.codeBlockFrames) this.codeBlockFrames = new Map()
    // Drop detached runners from prior lesson/hot-reload renders
    for (const [id, existing] of this.codeBlockFrames) {
      if (!existing.isConnected) this.codeBlockFrames.delete(id)
    }
    this.codeBlockFrames.set(containerId, frame)

    try {
      frame.srcdoc = this.buildCodeBlockSrcdoc(content, containerId)
    } catch (error) {
  ...
  loadJquery() {
    // Legacy helper retained for any external callers; opaque-frame code blocks
    // load jQuery inside the runner iframe instead.
